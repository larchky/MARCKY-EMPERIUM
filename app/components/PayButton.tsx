...
const DEFAULT_PAYMENT_EMAIL = "buyer@markyemporium.com";
...
function PayButton({
  ...
  const config = {
    ...
    meta: {
      order_source: "marky_emporium_cart_checkout",
  ...
  const openCheckout = () => {
    ...
    setTxRef(`MARKY-${Date.now()}-${Math.floor(Math.random() * 100000)}`);
    setIsOpen(true);
  };
        ...
        <div className="fixed inset-0 z-50 bg-primary/70 px-4">
          ...
          >
            ...
            <div className="mb-5 rounded-md border border-accent/30 bg-accent/10 p-4 text-sm leading-6 text-[#5c4214]">
              ...
              Marky Emporium to make arrangements for delivery.
            </div>
