      ...
      <div className="mx-auto max-w-7xl">
        <header className="flex flex-col justify-between gap-6 border-b border-accent/15 pb-8 md:flex-row md:items-center">
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              href="/"
              className="inline-flex w-fit items-center gap-2 rounded-md border border-accent/25 px-4 py-2 text-sm font-bold uppercase tracking-[0.14em] text-champagne transition hover:border-accent/60 hover:text-primary"
            >
        ...
        <section className="py-10">
          ...
          <p className="mt-4 max-w-2xl leading-7 text-champagne/72">
            Confirm your selected handbags, nightwear, accessories, and fashion
            pieces before making payment.
          </p>
        </section>

        {items.length === 0 ? (
          <section className="rounded-lg border border-accent/25 bg-white/85 p-8 text-center shadow-[0_18px_45px_rgba(216,60,134,0.10)]">
            <FiShoppingCart
            ...
            <Link
              href="/products"
              className="mt-6 inline-flex rounded-md bg-accent px-5 py-3 text-sm font-black uppercase tracking-[0.14em] text-white transition hover:bg-[#bf2d73]"
            >
              Browse Products
            </Link>
          </section>
          ...
          <section className="grid gap-6 lg:grid-cols-[1fr_24rem]">
            <div className="space-y-4">
              {items.map((item) => {
                ...
                return (
                  <article
                    key={item.id}
                    className="grid gap-4 rounded-lg border border-accent/15 bg-white/90 p-4 shadow-[0_18px_45px_rgba(216,60,134,0.10)] sm:grid-cols-[8rem_1fr]"
                  >
                  <div className="aspect-square overflow-hidden rounded-md border border-accent/15 bg-[#fff7fc]">
                    {item.imageUrl ? (
                      <img
                        alt={item.name}
                        src={item.imageUrl}
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-accent/15 pt-4">
                      <div className="inline-flex items-center rounded-md border border-accent/25 bg-white/80">
                        <button
                          type="button"
                          onClick={() =>
                          ...
                          className="grid size-10 place-items-center text-accent transition hover:text-primary"
                        >
                          <FiMinus aria-hidden="true" />
                        </button>
                        <span className="grid h-10 min-w-12 place-items-center border-x border-accent/15 px-3 font-black text-primary">
                          {item.quantity}
                        </span>
                        <button
                          type="button"
                          ...
                          className="grid size-10 place-items-center text-accent transition hover:text-primary disabled:cursor-not-allowed disabled:opacity-40"
                        >
            ...
            <aside className="h-fit rounded-lg border border-accent/25 bg-white/90 p-6 shadow-[0_30px_80px_rgba(216,60,134,0.14)]">
              <p className="text-sm font-black uppercase tracking-[0.22em] text-accent">
                Order Summary
              </p>

              <div className="mt-5 space-y-3 border-b border-accent/15 pb-5">
              ...
              <div className="mt-6 rounded-md border border-accent/30 bg-accent/10 p-4 text-sm leading-6 text-primary">
                Your cart is paid together. Marky Emporium will contact you to
              ...
              <div className="mt-6 grid gap-3">
                ...
                <Link
                  href="/products"
                  className="text-center text-sm font-bold uppercase tracking-[0.14em] text-accent transition hover:text-primary"
                >
                  Continue Shopping
                </Link>
              </div>
