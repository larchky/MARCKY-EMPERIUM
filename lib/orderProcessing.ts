...
function renderOrderText(order: ProcessedOrderResult["order"]) {
  return [
    "A new Marky Emporium order was placed.",
    "",
...
function renderOrderHtml(order: ProcessedOrderResult["order"]) {
  ...
  return `
    <div style="font-family: Arial, sans-serif; color: #17130b;">
      <h2 style="margin: 0 0 16px;">New Marky Emporium order</h2>
...
async function sendOrderNotification(order: ProcessedOrderResult["order"]) {
  ...
  const from =
    ...
    "Marky Emporium Orders <onboarding@resend.dev>";
  const recipients = to
    .split(",")
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from,
      to: recipients,
      subject: `New Marky Emporium order: ${order.product_name}`,
